...
class LogisticRegression:
    ...
    def _forward_prop(self, sample):
        ...
        self.A2 = 1/(1+np.exp(-1.0*self.Z2))
...
def generate_data():
    ...
    for i in range(10000):
        w1 = np.random.uniform(low=-10.0,high=10.0,size=(1,2))
        w2 = np.random.uniform(low=-10.0,high=10.0,size=(1,1))
        b1 = np.random.uniform(low=-10.0,high=10.0,size=(1,1))
...
if __name__ == "__main__":
    ...
    '''
        ...
        Initial bias: [-2.75 - -2.5], [-2.5 - -2.25]
        Initial weight: [-0.5 - 0.5, 1.5 - 2.5], [-8.0 - -7.0]
    '''
    train_time_list = []; train_acc_list = []
    test_time_list = []; test_acc_list = []
    # Calculate mean value(100 iteration)
    for i in range(100):
        w1_1 = np.random.uniform(low=-0.5,high=0.5,size=(1,1))
        w1_2 = np.random.uniform(low=1.5,high=2.5,size=(1,1))
        w1 = np.array([np.append(w1_1,w1_2)])
        w2 = np.random.uniform(low=-8.0,high=-7.0,size=(1,1))
        b1 = np.random.uniform(low=-2.75,high=-2.5,size=(1,1))
        b2 = np.random.uniform(low=-2.5,high=-2.25,size=(1,1))
        alpha = uniform(0.7,0.8)
